...
function getPINs(observed) {
  ...
  observed = removeDuplicates(observed);
  var possibleMatches = findPossibleMatches(observed);
  //var combos = combinations(possibleMatches);
  ...
  function findPossibleMatches(nums) {
    var possibles = [];

    for (var i = 0; i < nums.length; i++) {
        var index = Number(nums[i] - 1);
        if (index === undefined) index = 9;
        possibles.push(keypad[index - 3]);
        possibles.push(keypad[index - 1]);
        possibles.push(keypad[index]);
        possibles.push(keypad[index + 1]);
        possibles.push(keypad[index + 3]);
    }

    return removeDuplicates(possibles);
  }
  ...
  function removeDuplicates(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (uniques.indexOf(data[i]) === -1 && data[i] !== undefined) uniques.push(data[i]);
    }
