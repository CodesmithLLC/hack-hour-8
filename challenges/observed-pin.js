...
function getPINs(observed) {
  ...
  let num = parseInt(observed).split('');
  let arr = [];


  for (let x = 0; x < pinpad[0].length; x++){
    for (let y = 0; y < pinpad[1].length; y++){
      for (let z = 0; z < pinpad[2].length; z++){
        for (let a = 0; a < pinpad[3].length; a++){
          arr.push(pinpad[0][x]+pinpad[1][y]+pinpad[2][z]+pinpad[3][a]);
        }
      }
    }
  }
